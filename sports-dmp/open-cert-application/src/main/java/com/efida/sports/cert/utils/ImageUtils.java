...
import com.efida.sports.cert.service.certpic.impl.CertPicServiceImpl;
...
public final class ImageUtils {
    ...
    public static Font getSansFont(String fontName, int style,
                                   float fontSize) throws FontFormatException, IOException {
        Font font = null;
        InputStream is = CertPicServiceImpl.class
            .getResourceAsStream("/template/font/sans/SourceHanSansCN-Bold.otf");
        Font tempFont = Font.createFont(Font.TRUETYPE_FONT, is);
        font = tempFont.deriveFont(Font.BOLD, fontSize);
        is.close();
        System.out.println("获取字体名称-->:" + font.getFontName());
        return new Font("SourceHanSansCN-Bold", style, (int) fontSize);
    }
     ...
     * @methodName
     * @author wangyi
     * @param totalPx 占用区域像素
     * @param text 文字
     * @description
     * @return
     */
    @SuppressWarnings("restriction")
    public static int getCetStartPix(int totalPx, Font font, String text) {
        int startPx = 0;
        if (StringUtils.isBlank(text)) {
            return startPx;
        }
        FontDesignMetrics metrics = sun.font.FontDesignMetrics.getMetrics(font);
        int textpx = metrics.stringWidth(text);
        startPx = (totalPx / 2) - (textpx / 2);
        if (startPx < 0) {
            startPx = 0;
        }
        return startPx;
