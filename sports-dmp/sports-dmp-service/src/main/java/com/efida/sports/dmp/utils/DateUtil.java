...
/**
 * @author puyongyan
 * @version $Id: DateUtil.java, v 0.1 2017年11月22日 下午4:34:37 Exp $
 */
public class DateUtil {
     ...
     * @throws Exception 
     */
    public static boolean isEqualDay(Date date, Date compareDate) throws Exception {

        AssertUtils.assertNotNull(date, "date 不能为空");
        AssertUtils.assertNotNull(compareDate, "compareDate 不能为空");

        return date.getTime() / 86400 == compareDate.getTime() / 86400;
     ...
     * @throws Exception 
     */
    public static boolean compareDate(Date date, Date compareDate) throws Exception {
        AssertUtils.assertNotNull(date, "date 不能为空");
        AssertUtils.assertNotNull(compareDate, "compareDate 不能为空");
     ...
     * @throws Exception 
     */
    public static boolean compareDateNoSame(Date date, Date compareDate) throws Exception {
        AssertUtils.assertNotNull(date, "date 不能为空");
        AssertUtils.assertNotNull(compareDate, "compareDate 不能为空");
    ...
    public static List<String> getAvgDateList(Date startTime, Date endTime, int resultDays) {
        ...
        if (days < 1) {
            dates.add(format(startTime, WEB_FORMAT));
            return dates;
        }
        if (days <= resultDays) {
            while (true) {
        ...
        int interval = days / resultDays + 1;
        int num = 0;
        while (true) {
            if (startTime.after(endTime)) {
                if (dates.contains(format(endTime, WEB_FORMAT)) || num >= resultDays) {
                    break;
                } else {
                    dates.add(format(endTime, WEB_FORMAT));
                    break;
                }
            }
            num++;
            ...
            dates.add(format(startTime, WEB_FORMAT));
            startTime = addDay(startTime, interval);
        }
        return dates;
    }
