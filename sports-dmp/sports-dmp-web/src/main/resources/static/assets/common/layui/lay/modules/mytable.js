layui.define(['laytpl', 'laypage', 'layer', 'form'], function(exports){
  ...
  //字符常量
  ,MOD_NAME = 'mytable', ELEM = '.layui-table', THIS = 'layui-this', SHOW = 'layui-show', HIDE = 'layui-hide', DISABLED = 'layui-disabled', NONE = 'layui-none'
    ...
    if(options.url){ //Ajax请求
      ...
      $.ajax({
        ...
        ,success: function(res){
          if(that.fetchResponse(res, response.statusName) != response.statusCode){
            that.renderForm();
            return that.layMain.html('<div class="'+ NONE +'">'+ (that.fetchResponse(res, response.msgName) || '返回的数据状态异常') +'</div>');
          }
          that.renderData(res, curr, that.fetchResponse(res, response.countName)), sort();
          loadIndex && layer.close(loadIndex);
          typeof options.done === 'function' && options.done(res, curr, that.fetchResponse(res, response.countName));
        }
        ,error: function(e, m){
  ...
  Class.prototype.renderData = function(res, curr, count, sort){
    var that = this
    ,options = that.config
    ,data = that.fetchResponse(res, options.response.dataName) || []
    ,trs = []
    ,trs_fixed = []
    ,trs_fixed_r = []
