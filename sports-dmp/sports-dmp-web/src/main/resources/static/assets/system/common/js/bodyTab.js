...
console.log(changeRefreshStr,"changeRefresh1")

layui.define(["element","jquery"],function(exports){
	var element = layui.element,
		$ = layui.$,
		layId,changeRefreshStr
		Tab = function(){
			this.tabConfig = {
    ...
    Tab.prototype.navBar = function(strData){
        ...
        for(var i=0;i<data.length;i++){
            if(data[i].isOpen || data[i].isOpen == undefined){
	...
	Tab.prototype.changeRegresh = function(index){
        if(changeRefreshStr == "true"){//过滤掉右边点开导航不刷新
		...
		if(_this.attr("target") == "_blank"){
			...
			if(that.hasTab(_this.find("cite").text()) == -1 && _this.siblings("dl.layui-nav-child").length == 0){
				if($(".layui-tab-title.top_tab li").length == openTabNum){
					layer.msg('最多同时打开'+openTabNum+'个选项卡哦,您需要关闭之前的选项卡!');
					return;
				}
				tabIdIndex++;
				title += '<span class="tag-dot-inner"></span><cite>'+_this.find("cite").text()+'</cite>';
				...
				//当前窗口内容
				//左边导航在之前窗口打开，点击时做刷新
				refreshFrame($(".clildFrame .layui-tab-item.layui-show").find("iframe")[0]);
				var curmenu = {
					"icon" : _this.find("i.seraph").attr("data-icon")!=undefined ? _this.find("i.seraph").attr("data-icon") : _this.find("i.layui-icon").attr("data-icon"),
					"title" : _this.find("cite").text(),
					"href" : _this.attr("data-url")
	...
	/**重新加载iframe*/
	var refreshFrame=function(iframe){
