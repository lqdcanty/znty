        ...
        i.prototype.traverseModel = function(treeTable, parentNode, item, childrenAttrs) {
            ...
            layui.each(n, function(a, n) {
                ...
                var l = n.children && n.children.length > 0,
                    ...
                    s = o(["<li " + (a==0?'class="active"':"")+ (n.spread ? 'data-spread="' + n.spread + '"' : "") + ">", function() {
                    	 return l ? '<i class="fa '+n.icon+'">' + (n.spread ? t.arrow[1] : t.arrow[0]) + "</i>" : "";
                    }(), function() {
                        ...
                        return '<a href="' + (n.href || "javascript:;") + '" ' + (i.target && n.href ? 'target="' + i.target + '"' : "") + ">" + ('<i class="fa ' + n.icon + '">' +  "</i>") + ("<cite>" + (n.name || "未命名") + "</cite></a>")
                    }(), "</li>"].join(""));
            ...
            layui.each(n, function(a, n) {
                ...
                if (treeNode.level > 1) {
                    for (var ind = 1; ind < treeNode.level; ind++) {
                        indent += '<span style="display: inline-block;width: 40px;"></span>';
                    }
                }
                var p;
                        ...
                        function() {
                            ...
                            for (var ind = 0; ind < i.layout.length; ind++) {
                            	  if (i.layout[ind].treeNodes) {
                                      ret += '<td class="' + i.layout[ind].colClass + '" style="' + i.layout[ind].style + '"><li ' + (n.spread ? 'data-spread="' + n.spread + '"' : "") + '>' + (indent + (l ? '<i class="layui-icon layui-tree-spread">' + (n.spread ? t.arrow[1] : t.arrow[0]) + "</i>" : "")) + '<a href="' + (n.href || "javascript:;") + '" ' + (i.target && n.href ? 'target="' + i.target + '"' : "") + ">"  + ("<cite>" + (n.name || "未命名") + "</cite></a></li></td>");
