 ...
 * 
 * @author zoutao
 * @version $Id: ScoreVo.java, v 0.1 2018年8月15日 上午11:37:21 zoutao Exp $
 */
public class ScoreVo {
    ...
    */
    ...
    public static ScoreVo getVo(ScoreModel model) {
        if (model == null) {
            return null;
        }
        ScoreVo vo = new ScoreVo();
        vo.setEventName(model.getEventName());
        vo.setGender(model.getGender());
        vo.setGenderStr(model.getGenderStr());
        vo.setGroupName(model.getGroupName());
        vo.setMatchName(model.getMatchName());
        vo.setPartTime(model.getPartTime());
        vo.setPartTimeStr(model.getPartTimeStr());
        String scoreDesc = "";
        if (model.getScore() != null) {
            double doubleValue = model.getScore().doubleValue();
            long dev = (long) doubleValue;
            if (dev == doubleValue) {
                scoreDesc += dev;
            } else {
                scoreDesc += model.getScore().doubleValue();
            }

        }
        if (model.getScoreUnit() != null) {
            scoreDesc += model.getScoreUnit();
        }
        vo.setScoreDesc(
            StringUtils.isEmpty(model.getScoreDesc()) ? scoreDesc : model.getScoreDesc());
        vo.setSiteName(model.getSiteName());
        vo.setCompetitionCode(model.getCompetitionCode());
        vo.setPalyerCode(model.getPalyerCode());
        vo.setPalyerName(model.getPalyerName());
        vo.setRankIng(StringUtils.isEmpty(model.getRanking()) ? "-" : model.getRanking());
        return vo;
    }

    public static List<ScoreVo> getVos(List<ScoreModel> openScores) {
        List<ScoreVo> list = new ArrayList<ScoreVo>();
        if (openScores == null || openScores.size() < 1) {
            return list;
        }
        for (ScoreModel model : openScores) {
            ScoreVo vo = getVo(model);
            if (vo != null) {
                list.add(vo);
            }
        }
