...
public class PlayerVo implements Serializable {
    ...
    public static List<PlayerVo> getVos(Player player,
        ...
        List<PlayerVo> playerVos = getPlayerVo(player, athiete);
        return playerVos;
    }

    private static List<PlayerVo> getPlayerVo(Player player,
                                              SpAthletesEnrollModel athiete) throws Exception {
        ...
        for (FromTypeModel model : enrollForm) {
            ...
            if (playerMap.containsKey(key)) {
                ...
                if ("sex".equals(key) && StringUtils.isNotBlank(value)) {
                    formatVal = "m".equals(value) ? "男" : "女";
                }
                if ("playerCerType".equals(key) && StringUtils.isNotBlank(value)) {
                    formatVal = IdCardTypeEnum.getDescByCode(value);
                }
                vo.setVal(value);
                vo.setFormatVal(formatVal);
            } else {
                if (player != null && StringUtils.isNotBlank(player.getExtPro())) {
                    ...
                    if (parseObject.containsKey(key)) {
                        ...
                        vo.setFormatVal(parseObject.getString(key));
                    }
                }
            }
            vo.setFormatVal(convertFormatVal(vo.getFormatVal()));
            list.add(vo);
            ...
            if (model.getAttribute().equals("playerCerType") && model.getIsRequired()) {
                ...
                if (player != null) {
                    ...
                    playerCerNum.setFormatVal(player.getPlayerCerNo());
                }
        ...
        if (player != null && StringUtils.isNotBlank(player.getAttOne())) {
            ...
            vo.setFormatVal(convertFormatVal(player.getAttOne()));
            list.add(vo);
        }
        if (player != null && StringUtils.isNotBlank(player.getAttTwo())) {
            ...
            vo.setFormatVal(convertFormatVal(player.getAttTwo()));
            list.add(vo);
        }
        ...
        if (player != null && StringUtils.isNotBlank(player.getAttUrl())) {
            ...
            vo.setFormatVal(convertFormatVal(player.getAttUrl()));
            list.add(vo);
        }
     ...
     * @return
     */
    public static String convertFormatVal(String val){
        if(StringUtils.isBlank(val)){
            return "";
        }
        if(val.equalsIgnoreCase("(null)") || val.equalsIgnoreCase("<null>")){
            return "";
        }
        return val;
    }
